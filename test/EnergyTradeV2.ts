...
import { EnergyTradeV2, EnergyTradeV2__factory } from "../typechain-types";
import { Signer } from "ethers";

describe("EnergyTradeV2", function () {
  let energyTrade: EnergyTradeV2;
  let addr1: Signer;
  let addr2: Signer;
  let addr3: Signer;
  let addr4: Signer;
  ...
  beforeEach(async function () {
    ...
    energyTrade = await (
      await new EnergyTradeV2__factory(addr1).deploy(bucketDuration)
    ).waitForDeployment();
  });
  ...
  it("should allow the addr1 to close a bucket", async function () {
    await ethers.provider.send("evm_increaseTime", [900]);
    await ethers.provider.send("evm_mine", []);

    await energyTrade.connect(addr1).rollBucket();
  ...
  it("should roll a bucket with one match from one bid and ask", async function () {
    ...
    await expect(energyTrade.connect(addr1).rollBucket())
