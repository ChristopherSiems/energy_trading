...
contract EnergyTradeV2 is ReentrancyGuard {
  ...
  {
    return this.getTradeBucket(currBucketID - 1);
  }
  ...
  /// @dev Uses OpenZeppelin's `nonReentrant` modifier
  function rollBucket() external nonReentrant {
    require(
    ...
    for (uint256 i = 0; i < bidsSorted.length; i++) {
      (refunded, ) = payable(bidsSorted[i].traderAddr).call{
        value: (bidsSorted[i].energyAmount != 0)
  ...
  function markEnergySupplied(uint256 _bucketID, uint256 _tradeID) external {
    ...
    (bool sent, ) = payable(msg.sender).call{
      value: matchedTrades[_bucketID].matchedTrades[_tradeID].energyAmount *
